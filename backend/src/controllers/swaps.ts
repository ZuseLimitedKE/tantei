...
export class SwapsController {
    ...
    private async _getSwaps(args: GetAgentSwaps, agentController: AgentController, smartContract: SmartContract): Promise<SWAPS[]> {
        try {
            ...
            if (agentDetails) {
                ...
                swaps = await this.model.getSwapsFromDB({ user_evm_address: agentDetails.address });
                return swaps;
            } else {
        ...
        try {
            ...
            while (true) {
                ...
                if (!exists) {
                    ...
                    const oldest_swap = open_trades.find((t) => t.token_pair === s.token_pair);
    ...
    async getAgentTrades(args: GetAgentSwaps, agentController: AgentController, smartContract: SmartContract): Promise<AgentTrades[]> {
        try {
